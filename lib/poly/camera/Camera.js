...
var Camera = function () {
    function Camera() {
        ...
        this.inverseViewProj = _glMatrix.mat4.create();
        this.cameraDir = _glMatrix.vec3.create();
    ...
    _createClass(Camera, [{
        key: 'getRay',
        value: function getRay(pos, ray) {
            var proj = this.projectionMatrix;
            var view = this.matrix;

            _glMatrix.mat4.multiply(this.inverseViewProj, proj, view);
            _glMatrix.mat4.invert(this.inverseViewProj, this.inverseViewProj);

            _glMatrix.vec3.transformMat4(this.cameraDir, pos, this.inverseViewProj);
            _glMatrix.vec3.sub(this.cameraDir, this.cameraDir, this.position);
            _glMatrix.vec3.normalize(this.cameraDir, this.cameraDir);

            if (!ray) {
                ray = new _Ray2.default(this.position, this.cameraDir);
            } else {
